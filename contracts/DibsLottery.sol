...
contract DibsLottery is AccessControlUpgradeable {
	...
	function getUserTokensAndBalance(
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			balances[i] = _getUserTokneBalance(user, tokens[i]);
		}
	...
	function claimReward(address to) external onlyNotBlacklisted {
		...
		for (uint8 i = 0; i < _tokens.length; i++) {
			_withdraw(msg.sender, _tokens[i], to);
		}
	...
	function _findFirstValidIndex(address _user, address _token) internal view returns (uint256){
		uint256 start = balanceOfStartIndex[_user][_token];
		uint256 end = balanceOfEndIndex[_user][_token];
		
		while(start < end){
			if(balanceOf[_user][_token][start].deadline > _getToday()){
				return start;
			}
			start += 1;
		}
	...
	function _getUserTokneBalance(address _user, address _token) internal view returns (uint256) {
	...
	function _updateBalanceOstart(address _user, address _token) internal {
		balanceOfStartIndex[_user][_token] = _findFirstValidIndex(_user, _token);
	}
	...
	function _withdraw(
		...
		uint256 amount = _getUserTokneBalance(_user, _token);
		balanceOfStartIndex[_user][_token] = balanceOfEndIndex[_user][_token];
